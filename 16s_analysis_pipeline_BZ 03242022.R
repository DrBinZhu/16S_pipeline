...
# functions
{
    ...
    if (!is.na(ref_group)) {
      ...
      if (pheatmap_y == F) {
        group_dis_2_p = corrplot(distance_median,p.mat = group_dis_sig, method = 'shade', diag = F, type="upper",
                                 ...
                                 is.corr=FALSE, col=colorRampPalette(c("red","white"))(100)[c(10:100)],
  ...
  ##### One_factor_script #####
  One_factor_script = function(reads_table_all, metadata_all, ref_group, order) {
    reads_table = reads_table_all; metadata = metadata_all; factor_1 = colnames(metadata)[1]
    ...
    write.csv(beta$within_dis_sig,'within_group_distance.csv', row.names = T, quote = F)
    pdf("between_group_distance.pdf",width=2, height=2)
    beta$group_dis_2_p
    dev.off()
    beta$NMDS
    ...
    pdf("between_group_distance_2.pdf",width=3, height=3)
    beta = beta_diversity(reads_table, metadata = as.character(metadata[,1]), factor_name = factor_1, 
                          order = order, NMDS_skip = T, ref_group = ref_group, 
                          rarefy_to = NA, pheatmap_fontsize = 10, treeheight = 5, 
                          pheatmap_y = F)
    dev.off()
    ...
    save_pheatmap_pdf(network_result$p, "network.pdf", width=8, height=8)
    ...
    save_pheatmap_pdf(network_result$p, "network_abundant_taxa.pdf", width=6, height=6)
