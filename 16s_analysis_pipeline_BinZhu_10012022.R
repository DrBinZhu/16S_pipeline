...
# functions
{
  ...
  barplot <- function(reads_table, type_th = 0.1, taxa_num = 9, VMB = F) { # pass proportion data, samples on rows, taxa on columns.
  ...
  One_factor_script = function(reads_table, metadata, ref_group, order, VMB = parameter_28) {
    ...
    if (VMB == T) {
      for (a in 1: metadata_level) {
        ...
        barplot_result = barplot(reads_table_2, type_th = 0.1, taxa_num = 9) 
        barplot_result$p1
        ggsave(paste0('bar_plot', metadata_level_2[a], '_1.pdf') ,width=parameter_5, height=parameter_6)
        barplot_result$p2
        ggsave(paste0('bar_plot', metadata_level_2[a], '_2.pdf'),width=parameter_5, height=parameter_6)
      }
    }
