...
# functions
{
  ...
  dif_abundance <- function(reads_table,metadata, pvalue_th = 0.05, fold_change_th = 1, paired_test = F, order_reverse = F, style = 1, order = NA) {
    ...
    if (style == 1) {
      ...
      reads_table_abundance = get_abundance_table(reads_table, mc.cores = parameter_24)
...
if (ncol(metadata_all) == 1) {
  ...
  while (sum(!(name_1 %in% file_list)) == 0) {
    path_output = paste0(path_output,n)
...
#      parameter_27 = paste0("'",parameter_27,"'")
#      order = paste(parameter_27, collapse = ',')
#      order = paste0('c(', order, ')')
      order = parameter_27
    }
